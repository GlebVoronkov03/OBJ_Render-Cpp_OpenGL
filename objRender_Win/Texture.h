...
public:
	Texture(const char* fillName, GLenum type, GLint texture_unit)//конструктор текстур
	{
		this->type = type;
		...
		glTexParameteri(type, GL_TEXTURE_WRAP_S, GL_REPEAT);//S-координата x;
		glTexParameteri(type, GL_TEXTURE_WRAP_T, GL_REPEAT);//T-координата y;
	...
	~Texture()//диструктор текстур
	{
		glDeleteTextures(1, &this->id);
	}
	//динамический выбор текстурного блока
	...
	void loadFromFile(const char* fillName)
	{
		...
		glTexParameteri(this->type, GL_TEXTURE_WRAP_S, GL_REPEAT);//S-координата x;
		glTexParameteri(this->type, GL_TEXTURE_WRAP_T, GL_REPEAT);//T-координата y;
